...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_acc:
            ...
            print(f"New best model saved with accuracy: {best_acc:.4f}", flush=True)

    print("\nTraining complete.")
    print(f"Best validation accuracy: {best_acc:.4f}", flush=True)

    # --- Final Test ---
    print("\nLoading best model and running on the test set...", flush=True)
    model.load_state_dict(torch.load("best_model.pth"))
    
    test_loss, test_acc = validate(model, dataloaders['test'], criterion, device)
    
    print(f"Final Test Set Performance:", flush=True)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.data_path):
        print(f"Error: Data path not found: {args.data_path}", flush=True)
        print("Please check the --data_path argument or the default path in train.py", flush=True)
    else:
        main(args)